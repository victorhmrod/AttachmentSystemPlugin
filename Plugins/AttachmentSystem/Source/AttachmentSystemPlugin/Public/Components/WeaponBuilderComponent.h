 ...
 * @param IgnoredActor   Actor to ignore during collision (usually the parent).
 * @return true if the placement causes a collision, false if it's valid.
 */
	bool DoesCollideWithRail(const FTransform& TestTransform, USkeletalMeshComponent* ChildMesh, AActor* IgnoredActor) const;

	/** 
 * Disassembles the weapon by detaching and destroying all child attachments. 
 * 
 * - BlueprintCallable: Can be called at runtime (e.g., when player strips weapon).  
 ...
 * @return           Pointer to the found attachment, or nullptr if none exists.
 */
 ...
 *
 * @param Category   The attachment category to map (e.g., Barrel → "BarrelSocket").
...
private:
     ...
     * @return                  The first matching attachment, or nullptr if none found.
     */
