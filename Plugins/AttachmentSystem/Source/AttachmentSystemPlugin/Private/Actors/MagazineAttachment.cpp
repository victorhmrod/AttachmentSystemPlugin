...
void AMagazineAttachment::BeginPlay() { Super::BeginPlay(); }

void AMagazineAttachment::RunPerformanceTest() {
  constexpr int32 TestCount = 1'000'000;
  EBulletType Temp;

  UE_LOG(LogAttachmentSystem, Warning, TEXT("Performance Test: %d bullets"),
         TestCount);

  {
    ...
    UE_LOG(LogAttachmentSystem, Warning,
           TEXT("RingBuffer → %d insertions + removals in %.3f ms"), TestCount,
           DurationMs);
  }

  {
    ...
    UE_LOG(LogAttachmentSystem, Warning,
           TEXT("TQueue → %d insertions + removals in %.3f ms"), TestCount,
           DurationMs);
  }

  {
    ...
    UE_LOG(LogAttachmentSystem, Warning,
           TEXT("TArray → %d insertions + removals in %.3f ms"), TestCount,
           DurationMs);
  }
...
bool AMagazineAttachment::AddBullet(EBulletType BulletType) {
  ...
  if (BulletBuffer.Put(BulletType)) {
    UE_LOG(LogAttachmentSystem, Warning,
           TEXT("Magazine %s added bullet: %s (AmmoCount=%d)"), *GetName(),
           *BulletTypeToString(BulletType), GetAmmoCount());
...
void AMagazineAttachment::Server_AddBullet_Implementation(
...
EBulletType AMagazineAttachment::RemoveBullet() {
  ...
  if (BulletBuffer.Get(Out)) {
    UE_LOG(LogAttachmentSystem, Warning,
           TEXT("Magazine %s removed bullet: %s (AmmoCount=%d)"), *GetName(),
           *BulletTypeToString(Out), GetAmmoCount());
    ReplicatedAmmoCount = GetAmmoCount();
    return Out;
  }

  UE_LOG(LogAttachmentSystem, Warning, TEXT("Magazine %s is EMPTY!"),
         *GetName());
  return EBulletType::None;
}
...
bool AMagazineAttachment::IsEmpty() const { return BulletBuffer.IsEmpty(); }
...
FString AMagazineAttachment::BulletTypeToString(EBulletType Type) {
  ...
  case EBulletType::Standard_FMJ:
    return TEXT("Standard FMJ (Ball)");
  case EBulletType::ArmorPiercing:
    return TEXT("Armor Piercing (Black Tip)");
  case EBulletType::HollowPoint_SP:
    return TEXT("Hollow/Soft Point");
  case EBulletType::Tracer:
    return TEXT("Tracer (Green Tip)");
  case EBulletType::Subsonic:
    return TEXT("Subsonic");
  case EBulletType::Hunting_JSP:
    return TEXT("Jacketed Soft Point");
  default:
    return TEXT("Unknown");
  }
}
...
void AMagazineAttachment::RemoveBullets(int32 n) {
  ...
  {
    UE_LOG(LogAttachmentSystem, Warning,
           TEXT("Magazine %s fully emptied (%d rounds removed)."), *GetName(),
           n);
  } else if (BulletBuffer.IsEmpty()) {
    UE_LOG(LogAttachmentSystem, Warning, TEXT("Magazine %s already EMPTY."),
           *GetName());
  }
...
void AMagazineAttachment::LogBufferNonDestructive(const FString &Context) {
  ...
  if (Temp.Num() == 0) {
    ...
    for (int32 i = 0; i < Temp.Num(); i++) {
      UE_LOG(LogAttachmentSystem, Warning, TEXT("[%02d] %s"), i,
             *BulletTypeToString(Temp[i]));
    }
  }
...
void AMagazineAttachment::EnqueueNTimes(EBulletType Type, int32 Count) {
  for (int32 i = 0; i < Count; i++) {
    if (!BulletBuffer.Put(Type)) {
      UE_LOG(LogAttachmentSystem, Warning,
             TEXT("Buffer FULL at i=%d while adding %s"), i,
             *BulletTypeToString(Type));
      break;
    }
  }
}
...
void AMagazineAttachment::DequeueNTimes(int32 Count) {
  for (int32 i = 0; i < Count; i++) {
    EBulletType Out{};
    if (BulletBuffer.Get(Out)) {
      UE_LOG(LogAttachmentSystem, Warning, TEXT("Removed: %s"),
             *BulletTypeToString(Out));
    } else {
      UE_LOG(LogAttachmentSystem, Warning,
             TEXT("Tried to remove but buffer is EMPTY at i=%d"), i);
      break;
    }
  }
}