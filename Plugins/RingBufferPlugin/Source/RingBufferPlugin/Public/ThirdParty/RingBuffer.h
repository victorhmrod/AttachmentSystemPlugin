...
// include RELACY thread ordering header before the ringbuffer include to enable
// relacy testing
...
#define LARGE_RING_BLOCKS // define for larger block sizes - adds two counters,
                          // faster put/get
...
template <std::size_t N, typename IndexType> struct SlowRingMod {
...
// for our use, the 'if' is true most of the time, helping prediction where it
// matters
template <std::size_t N, typename IndexType> struct MidRingMod {
...
// FastRingMod is same as MidRingMod, but with specialized power of 2 case
// replaced with a mask

// template to detect if N is power of 2
template <std::size_t N> struct is_power_of_two {
  static const bool value = N && ((N & (N - 1)) == 0);
};

// same as MidRingMod, but with specialized power of 2 case replaced with a mask
template <std::size_t N, typename IndexType> struct FastRingModPowerOfTwo {
...
using FastRingMod = std::conditional_t<is_power_of_two<N>::value,
                                       FastRingModPowerOfTwo<N, IndexType>,
                                       MidRingMod<N, IndexType>>;

template <std::size_t N, typename DataType = char, typename IndexType = int32_t,
          typename RingMod = FastRingMod<N, IndexType>>
class RingBuffer {
  ...
  // "RingbufferIndexType should be lockfree for performance");
...
public:
  ...
  std::size_t AvailableToRead() const {
    return RingMod::Mod2N(2 * N + writeIndex_.load(NM::memory_order_acquire) -
                          readIndex_.load(NM::memory_order_acquire));
  }
  ...
  // removing undefined to call from any other thread
  std::size_t AvailableToWrite() const { return Size() - AvailableToRead(); }
  ...
  bool Put(const DataType &datum) { // paper above has ability to write bigger
                                    // blocks, is faster
  ...
  bool Get(DataType *data, std::size_t n) {
    ...
    if (RingMod::Mod2N(2 * N + pWriteIndex_ - r) <
        n) // predicted available to read
    {      // may not fit, check more exactly, costing an atomic read
      pWriteIndex_ = writeIndex_.load(NM::memory_order_acquire);
      if (RingMod::Mod2N(2 * N + pWriteIndex_ - r) <
          n)          // current available to read
...
#endif
private:
  // Taking counters mod N leaves one cell unused without additional fields to
  // track, but then atomic operations harder to check. Taking counters mod 2N
  // makes it possible to use all cells in the buffer when full, at additional
  // cost of bound enforcement on buffer access.
...
/* Notes:
        ...
        // todo - perhaps check if integer is unsigned? std::is_unsigned, needed
   to prevent subtraction errors want wraparound
        // todo - check if index is integer?
        // todo - asserts throughout
        // see Correct and Efficient Bounded FIFO Queues,
   https://www.irif.fr/~guatto/papers/sbac13.pdf
        // and https://www.snellman.net/blog/archive/2016-12-13-ring-buffers/
        // also Facebook Folly threading lib example
