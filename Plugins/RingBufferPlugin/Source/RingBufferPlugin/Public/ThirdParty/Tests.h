...
struct Stats {
  Stats(const char *testName, const char *typeName, size_t bufferSize,
        size_t blockSize, long bytesPerPass) {
#ifndef SAMD21_BUILD
    passCount = 100;
#else
    passCount = 10;
...
inline void ShowLogFormat() {
  printf("Test, ring size, block transfer size, avg MB/s, max MB/s, min MB/s, "
         "success/fail, buffer name, avgMs, \n");
}

long ZeroToOne(long v) {
  if (v != 0)
    return v;
  return 1;
}
...
inline void Log(const Stats &stats) {
  ...
  auto avgMB10 =
      (10000 * stats.bytesPerPass / ZeroToOne(avgElapsedMs)) / (1UL << 20);
  auto minMB10 =
      (10000 * stats.bytesPerPass / ZeroToOne(stats.maxMs)) / (1UL << 20);
  auto maxMB10 =
      (10000 * stats.bytesPerPass / ZeroToOne(stats.minMs)) / (1UL << 20);
  ...
  sprintf(buffer,
          "%s, %ld, %ld, %lld.%lld, %lld.%lld, %lld.%lld, %d, %s, %ld, ",
          stats.testName, (long)stats.bufferSize, (long)stats.blockSize,
          avgMB10 / 10, avgMB10 % 10, minMB10 / 10, minMB10 % 10, maxMB10 / 10,
          maxMB10 % 10, stats.success, name, (long)avgElapsedMs);
  WriteLine(buffer);
#else
  ...
  for (int pass = 0; pass < stats.passCount; ++pass) {
    ...
    std::thread t1([&]() {
      ...
      while (processed < size) {
        ...
        while (rb.AvailableToWrite() < M) { // spin
        }
    ...
    std::thread t2([&]() {
      ...
      while (processed < size) {
        while (rb.AvailableToRead() < M) { // spin
        }
...
bool ThroughputDouble(long size) {
  ...
  for (int pass = 0; pass < stats.passCount; ++pass) {
    ...
    std::thread t1([&]() {
      ...
      while (processed < size) {
        for (auto i = 0; i < M; ++i) {
          while (rb.AvailableToWrite() < 1) { // spin
          }
    ...
    std::thread t2([&]() {
      ...
      while (processed < size) {
        for (auto i = 0; i < M; ++i) {
          while (rb.AvailableToRead() < 1) { // spin
          }
...
template <size_t N> bool FinalStressTest() {
  ...
  std::thread t1([&]() {
    ...
    while (!stop1) {
      ...
      if (rnd1.Next() & 1) { // single bytes
        for (auto j = 0; j < size; ++j) {
          errors1 += !rb.Put(buffer[writer]);
          writer = (writer + 1) & 1023;
        }
      } else { // block at a time
  ...
  std::thread t2([&]() {
    ...
    while (true) {
      ...
      for (auto check = 0; check < size; ++check)
        errors2 += buffer[(start + check) & 2047] !=
                   buffer[(start + check + 1024) & 2047];
