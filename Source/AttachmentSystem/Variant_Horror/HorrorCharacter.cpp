...
AHorrorCharacter::AHorrorCharacter() {
  ...
  SpotLight->SetRelativeLocationAndRotation(FVector(30.0f, 17.5f, -5.0f),
                                            FRotator(-18.6f, -1.3f, 5.26f));
...
void AHorrorCharacter::BeginPlay() {
  ...
  GetWorld()->GetTimerManager().SetTimer(SprintTimer, this,
...
void AHorrorCharacter::SetupPlayerInputComponent(
...
void AHorrorCharacter::SprintFixedTick() {
  // are we out of recovery, still have stamina and are moving faster than our
  // walk speed?
  if (bSprinting && !bRecovering && GetVelocity().Length() > WalkSpeed) {
    ...
    if (SprintMeter >= SprintTime) {
      ...
      // set the walk or sprint speed depending on whether the sprint button is
      // down
      GetCharacterMovement()->MaxWalkSpeed =
          bSprinting ? SprintSpeed : WalkSpeed;
