...
void APawn_Smith::BeginPlay() {
  ...
  CurrentAttachment =
      GetWorld()->SpawnActor<AAttachment_Base>(FirstAttachmentClass);
...
void APawn_Smith::Tick(float DeltaTime) {
  ...
  PlayerController->GetMousePosition(LocalMouseLocation.X,
                                     LocalMouseLocation.Y);

  PlayerController->DeprojectMousePositionToWorld(WorldSpaceMouseLocation,
                                                  WorldSpaceMouseDirection);

  if (CurrentAttachment)
    CheckForSnapping();

  // Check for matching railings tags to place Attachment, if not matching, set
  // Attachment materials to denied
  if (HitRailing && bIsSnapping && CurrentAttachment &&
      !CurrentAttachment->IsCollidingAttachment() &&
      !CurrentAttachment->IsCollidingRailing()) {
  ...
  if (CurrentAttachment) {
    if (CurrentAttachment->IsCollidingRailing()) {
      GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Red,
                                       TEXT("Colliding Railing"));
    } else {
      GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Green,
                                       TEXT("Not Colliding Railing"));
    }

    if (CurrentAttachment->IsCollidingAttachment()) {
      GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Red,
                                       TEXT("Colliding Attachment"));
    } else {
      GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Green,
                                       TEXT("Not Colliding Attachment"));
    }
  }
}

void APawn_Smith::SetupPlayerInputComponent(
  ...
  if (UEnhancedInputComponent *EnhancedInputComponent =
    ...
    EnhancedInputComponent->BindAction(InteractAction, ETriggerEvent::Started,
                                       this, &APawn_Smith::OnInteract);

    EnhancedInputComponent->BindAction(ActivateRotateAction,
                                       ETriggerEvent::Started, this,
                                       &APawn_Smith::OnPressedCamRotation);
    EnhancedInputComponent->BindAction(ActivateRotateAction,
                                       ETriggerEvent::Completed, this,
                                       &APawn_Smith::OnReleasedCamRotation);

    EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered,
                                       this, &APawn_Smith::OnLook);

    EnhancedInputComponent->BindAction(ScrollAction, ETriggerEvent::Triggered,
                                       this, &APawn_Smith::OnScroll);

    EnhancedInputComponent->BindAction(ActionAction, ETriggerEvent::Started,
                                       this, &APawn_Smith::OnAction);

    EnhancedInputComponent->BindAction(MirrorAttachmentAction,
                                       ETriggerEvent::Started, this,
                                       &APawn_Smith::OnMirrorAttachment);
  }
}
...
void APawn_Smith::CheckForSnapping() {
  ...
  // GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Red,
  // StartLoc.ToString());
  ValidMousePos = WorldSpaceMouseLocation +
                  (WorldSpaceMouseDirection * SpringArm->TargetArmLength * 2.f);
  // GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Green,
  // ValidMousePos.ToString());

  // Line trace to check possible collision with railings
  UKismetSystemLibrary::SphereTraceSingle(
      GetWorld(), StartLoc, ValidMousePos, CurrentAttachment->Radius,
      TraceTypeQuery4, false, ActorsToIgnore, EDrawDebugTrace::ForOneFrame,
      HitResult, true);

  // Check if Hit is Detected
  if (HitResult.bBlockingHit) {
    // GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Cyan,
    // FString("HitObject : %s").Append(*HitResult.GetActor()->GetName()));
    if (!CurrentAttachment->IsCollidingRailing()) {
      // Get Hit Railing
      ...
      UKismetSystemLibrary::SphereTraceSingle(
          GetWorld(), HitResult.Location, HitResult.Location, 10.f,
          TraceTypeQuery4, false, NewActorsToIgnore,
          EDrawDebugTrace::ForOneFrame, NewHitResult, true);

      if (NewHitResult.bBlockingHit &&
          NewHitResult.GetActor() != HitResult.GetActor()) {
        if (HitRailing = Cast<AWeapon_Railing>(NewHitResult.GetActor())) {
          ...
          if (UGameplayStatics::ProjectWorldToScreen(
                  PlayerController,
                  NewHitResult.ImpactPoint +
                      NewHitResult.GetActor()->GetActorUpVector() * 5.f,
                  NewMouseLoc) &&
              !CurrentAttachment->IsCollidingRailing()) {
...
void APawn_Smith::CursorSnapping() {
  ...
  const float StartY = HitRailing->StartPoint->GetComponentLocation().Y <
                               HitRailing->EndPoint->GetComponentLocation().Y
                           ? HitRailing->StartPoint->GetComponentLocation().Y
                           : HitRailing->EndPoint->GetComponentLocation().Y;
  const float EndY = HitRailing->StartPoint->GetComponentLocation().Y <
                             HitRailing->EndPoint->GetComponentLocation().Y
                         ? HitRailing->EndPoint->GetComponentLocation().Y
                         : HitRailing->StartPoint->GetComponentLocation().Y;
  //
  ...
  const float tempY = FMath::GetMappedRangeValueClamped(
      UE::Math::TVector2(StartY, EndY), UE::Math::TVector2(0.f, 1.f), MouseY);
  ...
  const float FinalY =
      FMath::Clamp(EndY - CurrentAttachment->BaseOffset,
                   StartY + CurrentAttachment->BaseOffset, NewAttachmentLoc.Y);
  NewAttachmentLoc.Y = FinalY;

  FRotator NewAttachmentRot = FRotator(HitRailing->GetActorRotation().Pitch,
                                       HitRailing->GetActorRotation().Yaw,
                                       HitRailing->GetActorRotation().Roll);

  // GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Red,
  // NewAttachmentRot.ToString());

  // GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Red, FString("Rail Vector
  // : ").Append(HitRailing->GetActorUpVector().ToString()));
  // GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Red, FString("Default Up
  // Vector : ").Append(FVector::UpVector.ToString()));
  // GEngine->AddOnScreenDebugMessage(-1, 0.f, FColor::Red, FString("DotProduct
  // :
  // ").Append(FString::SanitizeFloat(FVector::DotProduct(HitRailing->GetActorUpVector(),
  // FVector::UpVector))));

  // Force Rotation due to Rotator Limitation if Railing is Upside Down
  if (FMath::IsNearlyEqual(FVector::DotProduct(HitRailing->GetActorUpVector(),
                                               FVector::UpVector),
                           -1.f, 0.1f)) {
...
void APawn_Smith::OnInteract(const FInputActionValue &Value) {
  ...
  if (CurrentAttachment && bCanAttachmentBePlaced && bIsSnapping &&
      !CurrentAttachment->IsCollidingAttachment() &&
      !CurrentAttachment->IsCollidingRailing()) {
    // Spawn New Actor to attach it to Railing
    const TObjectPtr<AAttachment_Base> tempNewAttachment =
        GetWorld()->SpawnActor<AAttachment_Base>(CurrentAttachment->GetClass());
    tempNewAttachment->SetActorLocationAndRotation(AttachmentLocation,
                                                   AttachmentRotation);
    tempNewAttachment->SetActorScale3D(CurrentAttachment->GetActorScale3D());
    tempNewAttachment->AttachToComponent(
        HitRailing->RailMesh, FAttachmentTransformRules::KeepWorldTransform);
    ...
    if (!DoOnceChangeAttachment || true) {
      ...
      // Spawn First Attachment (Magni)
      CurrentAttachment =
          GetWorld()->SpawnActor<AAttachment_Base>(SecondAttachmentClass);
    }
    ...
    ValidMousePos =
        WorldSpaceMouseLocation +
        (WorldSpaceMouseDirection * SpringArm->TargetArmLength * 2.f);

    // Line trace to check possible collision with Attachment
    UKismetSystemLibrary::LineTraceSingle(
        GetWorld(), StartLoc, ValidMousePos, TraceTypeQuery3, false,
        ActorsToIgnore, EDrawDebugTrace::None, HitResult, true);

    // Check for Hit Result, then move camera to hit attachment, return to
    // origin otherwise
    if (HitResult.bBlockingHit) {
      if (FocusedAttachment = Cast<AAttachment_Base>(HitResult.GetActor())) {
        ...
        FocusEndLoc = FocusedAttachment->GetActorLocation() +
                      FocusedAttachment->CamPositionOffset;
...
void APawn_Smith::OnLook(const FInputActionValue &Value) {
  if (bEnableCamRotation) {
    ...
    // FRotator(LookVector.Y * 100, LookVector.X * 100, 0.f).ToString());
  }
}
...
void APawn_Smith::OnMirrorAttachment(const FInputActionValue &Value) {
  if (CurrentAttachment) {
    ...
    CurrentAttachment->SetActorScale3D(FVector(CurrentAttachmentScale.X * -1.f,
                                               CurrentAttachmentScale.Y,
                                               CurrentAttachmentScale.Z));
  }
}
#pragma endregion
