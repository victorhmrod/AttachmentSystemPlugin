...
void AAttachment_Magni::BeginPlay() {
  ...
  MagniLensMaterial->SetTextureParameterValue(FName("RenderTarget"),
                                              RenderTarget2D->TextureTarget);
...
void AAttachment_Magni::UpdateMagniMovement(const float Alpha) {
  Mesh->SetRelativeRotation(
      FMath::Lerp(OriginalRotation, FinalRotation, Alpha));
}

// Function Bound to parent delegate
void AAttachment_Magni::SetMagniMat(const bool bActivate) {
  bActivate ? Mesh->SetMaterial(0, DeniedMaterial)
            : Mesh->SetMaterial(0, MagniBodyMaterial);
}
