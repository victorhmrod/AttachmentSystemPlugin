...
AShooterWeapon::AShooterWeapon() {
  ...
  FirstPersonMesh->SetFirstPersonPrimitiveType(
      EFirstPersonPrimitiveType::FirstPerson);
  FirstPersonMesh->bOnlyOwnerSee = true;

  // create the third person mesh
  ThirdPersonMesh =
      CreateDefaultSubobject<USkeletalMeshComponent>(TEXT("Third Person Mesh"));
  ThirdPersonMesh->SetupAttachment(RootComponent);

  ThirdPersonMesh->SetCollisionProfileName(FName("NoCollision"));
  ThirdPersonMesh->SetFirstPersonPrimitiveType(
...
void AShooterWeapon::StartFiring() {
  ...
  // this may be under the refire rate if the weapon shoots slow enough and the
  // player is spamming the trigger
  ...
  if (TimeSinceLastShot > RefireRate) {
    ...
    if (bFullAuto) {
      GetWorld()->GetTimerManager().SetTimer(
          RefireTimer, this, &AShooterWeapon::Fire, TimeSinceLastShot, false);
    }
  }
}
...
void AShooterWeapon::Fire() {
  ...
  MakeNoise(ShotLoudness, PawnOwner, PawnOwner->GetActorLocation(),
  ...
  if (bFullAuto) {
    ...
    GetWorld()->GetTimerManager().SetTimer(
        RefireTimer, this, &AShooterWeapon::Fire, RefireRate, false);
  } else {
...
FTransform AShooterWeapon::CalculateProjectileSpawnTransform(
  ...
  const FVector SpawnLoc =
      MuzzleLoc + ((TargetLocation - MuzzleLoc).GetSafeNormal() * MuzzleOffset);
