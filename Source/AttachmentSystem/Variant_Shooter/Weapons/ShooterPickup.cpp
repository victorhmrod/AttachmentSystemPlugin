...
AShooterPickup::AShooterPickup() {
  ...
  SphereCollision =
      CreateDefaultSubobject<USphereComponent>(TEXT("Sphere Collision"));
...
void AShooterPickup::OnConstruction(const FTransform &Transform) {
  ...
  if (FWeaponTableRow *WeaponData =
          WeaponType.GetRow<FWeaponTableRow>(FString())) {
    // set the mesh
...
void AShooterPickup::OnOverlap(UPrimitiveComponent *OverlappedComponent,
                               AActor *OtherActor,
  ...
  if (IShooterWeaponHolder *WeaponHolder =
    ...
    GetWorld()->GetTimerManager().SetTimer(
        RespawnTimer, this, &AShooterPickup::RespawnPickup, RespawnTime, false);
  }
}
