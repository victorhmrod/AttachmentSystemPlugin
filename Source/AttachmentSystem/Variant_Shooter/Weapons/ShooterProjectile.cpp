...
AShooterProjectile::AShooterProjectile() {
  ...
  RootComponent = CollisionComponent =
      CreateDefaultSubobject<USphereComponent>(TEXT("Collision Component"));
  ...
  // create the projectile movement component. No need to attach it because it's
...
void AShooterProjectile::NotifyHit(class UPrimitiveComponent *MyComp,
                                   AActor *Other,
  ...
  MakeNoise(NoiseLoudness, GetInstigator(), GetActorLocation(), NoiseRange,
            NoiseTag);
  ...
  if (DeferredDestructionTime > 0.0f) {
    GetWorld()->GetTimerManager().SetTimer(
        DestructionTimer, this, &AShooterProjectile::OnDeferredDestruction,
...
void AShooterProjectile::ExplosionCheck(const FVector &ExplosionCenter) {
  ...
  GetWorld()->OverlapMultiByObjectType(Overlaps, ExplosionCenter,
  ...
  for (const FOverlapResult &CurrentOverlap : Overlaps) {
    // overlaps may return the same actor multiple times per each component
    // overlapped ensure we only damage each actor once by adding it to a
    // damaged list
    if (DamagedActors.Find(CurrentOverlap.GetActor()) == INDEX_NONE) {
      ...
      const FVector &ExplosionDir =
          CurrentOverlap.GetActor()->GetActorLocation() - GetActorLocation();

      // push and/or damage the overlapped actor
      ProcessHit(CurrentOverlap.GetActor(), CurrentOverlap.GetComponent(),
                 GetActorLocation(), ExplosionDir.GetSafeNormal());
    }
  }
}

void AShooterProjectile::ProcessHit(AActor *HitActor,
