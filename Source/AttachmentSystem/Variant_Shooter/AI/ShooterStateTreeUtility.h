...
struct FStateTreeLineOfSightToTargetConditionInstanceData {
  ...
  /** Number of vertical line of sight checks to run to try and get around low
   * obstacles */
  UPROPERTY(EditAnywhere, Category = "Condition")
...
struct FStateTreeLineOfSightToTargetCondition
  ...
  virtual bool
  TestCondition(FStateTreeExecutionContext &Context) const override;
  ...
  GetDescription(const FGuid &ID, FStateTreeDataView InstanceDataView,
                 ...
                 EStateTreeNodeFormatting Formatting =
                     EStateTreeNodeFormatting::Text) const override;
#endif
};
...
/**
 *  StateTree task to have an NPC process AI Perceptions and sense nearby
 * enemies
 */
