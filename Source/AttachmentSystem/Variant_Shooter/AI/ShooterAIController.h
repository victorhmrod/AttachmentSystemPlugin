...
DECLARE_DELEGATE_TwoParams(FShooterPerceptionUpdatedDelegate, AActor *,
                           const FAIStimulus &);
 ...
 */
UCLASS(abstract)
class ATTACHMENTSYSTEM_API AShooterAIController : public AAIController {
  GENERATED_BODY()
...
public:
  /** Called when an AI perception has been updated. StateTree task delegate
   * hook */
  FShooterPerceptionUpdatedDelegate OnShooterPerceptionUpdated;

  /** Called when an AI perception has been forgotten. StateTree task delegate
   * hook */
...
protected:
  /** Called when the AI perception component updates a perception on a given
   * actor */
  UFUNCTION()
