...
void AShooterPlayerController::BeginPlay() {
  ...
  if (IsLocalPlayerController()) {
    ...
    BulletCounterUI =
        CreateWidget<UShooterBulletCounterUI>(this, BulletCounterUIClass);
    ...
    if (BulletCounterUI) {
      ...
      UE_LOG(LogAttachmentSystem, Error,
             TEXT("Could not spawn bullet counter widget."));
    }
  }
}
...
void AShooterPlayerController::OnPossess(APawn *InPawn) {
  ...
  if (AShooterCharacter *ShooterCharacter = Cast<AShooterCharacter>(InPawn)) {
    ...
    ShooterCharacter->OnDamaged.AddDynamic(
        this, &AShooterPlayerController::OnPawnDamaged);
...
void AShooterPlayerController::OnPawnDestroyed(AActor *DestroyedActor) {
  ...
  if (ActorList.Num() > 0) {
    ...
    if (AShooterCharacter *RespawnedCharacter =
            GetWorld()->SpawnActor<AShooterCharacter>(CharacterClass,
                                                      SpawnTransform)) {
...
void AShooterPlayerController::OnBulletCountUpdated(int32 MagazineSize,
                                                    int32 Bullets) {
  // update the UI
  if (BulletCounterUI) {
